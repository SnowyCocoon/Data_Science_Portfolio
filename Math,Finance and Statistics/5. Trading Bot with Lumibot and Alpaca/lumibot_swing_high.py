...
#https://www.tradingsim.com/day-trading/swing-high
#Swing high is when price makes a high and is followed by two consecutive lower highs.
class SwingHigh(Strategy):
    data = []
    order_number = 0
    def initialize(self, parameters=None):
        self.sleeptime = "10S"
        #self.before_market_opens = "5M"
    ...
    def on_trading_iteration(self):
        ...
        if len(self.data) > 3:
            ...
            if temp[-1] > temp[1] > temp[0]:
                self.log_message("Last 3 prints: {temp}")
                order = self.create_order(symbol, quantity=10, side="buy")
                self.submit_order(order)
                self.order_number += 1
                if self.order_number == 1:
                    self.log_message(f"Entry price: {temp[-1]}")
                    entry_price = temp[-1] #filled
            if self.get_position(symbol) and self.data[-1] < entry_price * 0.955:
                self.sell_all()
                self.order_number = 0
